...
def get_output_files(wildcards):
    ...
    if config["end"] == "norm":
        for t in config["tissues"]:
            files.append(config["datadir"]+"/"+t["name"]+"/plots/deseq2/pca_score.png")
            files.append(config["datadir"]+"/"+t["name"]+"/plots/tpm/pca_score.png")
...
def get_plots_dir(wildcards):
    return f'{get_tissue_dir(wildcards)}/plots/{wildcards.plots_type}'
